<template>
  <div class="issuing-page">
    <header class="issuing-topbar">
      <div class="issuing-topbar-left">
       <button class="back-btn" @click="goBackToLanding">
  ← Back to landing
</button>
        <div>
          <div class="small-topline">Cards</div>
          <div class="top-title">Issuing API Explorer</div>
        </div>
      </div>

      <div class="top-tags">
        <span class="top-tag">Cards platform</span>
        <span class="top-tag">iCard blue theme</span>
      </div>
    </header>

    <div class="issuing-layout">
      <aside class="issuing-sidebar">
        <div class="sidebar-brand">
          <img src="/logo.png" alt="iCard logo" class="sidebar-logo" />
          <div>
            <div class="sidebar-brand-subtitle">Issuing API documentation</div>
          </div>
        </div>
      ...
      <main class="issuing-main">
        <div class="content-hero">
          <div class="content-topline">
            <span class="hero-chip">Issuing API</span>
...
import "./IssuingApiExplorer.css";
import { issuingApiMenu, issuingApiContent } from "../data/issuingApiData";
...
const filteredMenu = computed(() => {
  if (!query.value.trim()) return issuingApiMenu;

  const q = query.value.toLowerCase();

  return issuingApiMenu
    .map((group) => ({
      ...group,
...
const allItems = computed(() => issuingApiMenu.flatMap((group) => group.items));

const activeMeta = computed(() => {
  return allItems.value.find((item) => item.id === activeId.value) || allItems.value[0];
});

const active = computed(() => issuingApiContent[activeId.value]);
</script>