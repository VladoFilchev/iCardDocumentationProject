<template>
  <div class="merchant-page">
    <header class="merchant-topbar">
      <div class="merchant-topbar-left">
  ...
  ← Back to landing
        ...
        <div>
          ...
          <div class="top-title">Merchant API Explorer</div>
        </div>
      </div>

      <div class="top-tags">
        <span class="top-tag">Onboarding API</span>
      ...
      <main class="merchant-main">
        <div class="content-hero">
          <div class="content-topline">
            <span class="hero-chip">Merchant API</span>
...
import "./MerchantApiExplorer.css";
import { merchantApiMenu, merchantApiContent } from "../data/merchantApiData";
...
const filteredMenu = computed(() => {
  if (!query.value.trim()) return merchantApiMenu;

  const q = query.value.toLowerCase();

  return merchantApiMenu
    .map((group) => ({
      ...group,
...
const allItems = computed(() => merchantApiMenu.flatMap((group) => group.items));

const activeMeta = computed(() => {
  return allItems.value.find((item) => item.id === activeId.value) || allItems.value[0];
});

const active = computed(() => merchantApiContent[activeId.value]);
</script>